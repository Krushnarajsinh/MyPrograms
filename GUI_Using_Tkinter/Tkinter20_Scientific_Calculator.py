def on_click(event):
    global screen_val
    text=event.widget.cget("text")  #By using event.widget we can get that button and cget() is used to get text on that button
    #print(text)
    val=screen_val.get()   #To get value from the screen
    result=''
    ...
    elif text=="C":  #To Cler The screen
        screen_val.set("")
        screen.update()

    elif text=="DEL":
        last = val[:len(val) - 1]  #here we decrement the val(this val variable stores screen values as a string) string by 1
        screen_val.set(last)
        screen.update()
